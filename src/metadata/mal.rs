...
impl MalClient {
    pub fn new(client_id: String) -> Self {
        ...
        headers.insert("X-MAL-CLIENT-ID", header::HeaderValue::from_str(&client_id).unwrap());
...
impl MetadataProvider for MalClient {
    async fn search(&self, query: &str) -> Result<Vec<AnimeMetadata>> {
        ...
        let response = self.client.get(&url)
            .query(&[("q", query), ("limit", "5"), ("fields", "start_date,end_date,mean,status,num_episodes,synopsis,main_picture,genres")])
            .send()
            .await?; // Removed .context() as we are using crate::error::Error

        if !response.status().is_success() {
            return Err(Error::Metadata(format!("MAL API Error: {}", response.status())));
        }

        let resp_json: MalSearchResponse = response.json().await?;
        
        let results = resp_json.data.into_iter().map(|node| {
            let a = node.node;
            AnimeMetadata {
                id: a.id,
                title: a.title,
                ...
                genres: a.genres.map(|g| g.into_iter().map(|ge| ge.name).collect()).unwrap_or_default(),
            }
        }).collect();
    ...
    async fn get_details(&self, id: u64) -> Result<AnimeMetadata> {
        let url = format!("{}/anime/{}", MAL_API_BASE, id);
         let response = self.client.get(&url)
            .query(&[("fields", "start_date,end_date,mean,status,num_episodes,synopsis,main_picture,genres")])
            .send()
            .await?;
